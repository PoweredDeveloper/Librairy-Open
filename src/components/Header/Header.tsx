...
import { FaChartArea, FaCube, FaFingerprint, FaSquareCheck, FaX } from 'react-icons/fa6'
...
const products = [
  { name: 'Analytics', description: 'Get a better understanding of your traffic', href: '#', icon: FaChartArea },
  { name: 'Engagement', description: 'Speak directly to your customers', href: '#', icon: FaCube },
  { name: 'Security', description: 'Your customers’ data will be safe and secure', href: '#', icon: FaFingerprint},
  { name: 'Integrations', description: 'Connect with third-party tools', href: '#', icon: FaSquareCheck },
  { name: 'Automations', description: 'Build strategic funnels that will convert', href: '#', icon: FaAd },
]
    ...
    <header className="bg-white">
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <Popover.Group className="hidden lg:flex lg:gap-x-12">
          {links.map((link:IHeaderButtonType) => <Link href={link.link} className="text-sm font-semibold leading-6 text-brown-900" key={uuidv4()}>{link.title}</Link>)}
          <Popover className="relative">
            <Popover.Button className="flex items-center gap-x-1 text-sm font-semibold leading-6 text-brown-900">
              Продукт
              <FaChevronDown className="h-5 w-5 flex-none text-brown-400" aria-hidden="true" />
            </Popover.Button>
                  ...
                  {products.map((item) => (
                    ...
                    >
                      <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-brown-50 group-hover:bg-white">
                        <item.icon className="h-6 w-6 text-brown-600 group-hover:text-orange-600" aria-hidden="true" />
                      </div>
                      <div className="flex-auto">
                        <Link href={item.href} className="block font-semibold text-brown-900">
                          {item.name}
                          <span className="absolute inset-0" />
                        </Link>
                        <p className="mt-1 text-brown-600">{item.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          <Link href="/auth/login" className="text-sm font-semibold leading-6 text-brown-900">
